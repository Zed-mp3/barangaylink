<?php
// public/user/check_dashboard_updates.php
...
try {
    ...
    // Check for new/changed requests
    $new_requests_sql = "SELECT COUNT(*) as count FROM service_requests 
                         WHERE user_id = {$user['id']} AND UNIX_TIMESTAMP(created_at) > $last_update";
    $new_requests = $db->query($new_requests_sql)->fetch_assoc()['count'];
    
    // Check for updated requests (status changes)
    $updated_requests_sql = "SELECT COUNT(*) as count FROM service_requests 
                             WHERE user_id = {$user['id']} AND UNIX_TIMESTAMP(updated_at) > $last_update";
    $updated_requests = $db->query($updated_requests_sql)->fetch_assoc()['count'];
    
    $total_updates = $new_announcements + $new_requests + $updated_requests;
    ...
    $current_last_update_sql = "SELECT MAX(GREATEST(
        ...
        COALESCE(UNIX_TIMESTAMP((
            SELECT MAX(updated_at) FROM service_requests WHERE user_id = {$user['id']}
        )), 0)
    )) as last_update";
